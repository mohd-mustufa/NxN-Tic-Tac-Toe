...
const checkDraw = () => {
    return [...cells].every((cell) => cell.innerHTML === "X" || cell.innerHTML === "O");
};
...
const checkWinFor3x3Board = (n, currentTurn) => {
    ...
    for (let i = 0; i < n * n; i += n) {
        if (checkLine(i, 1, n, currentTurn))
            return true;
    }
    // Check for cols
    for (let i = 0; i < n; i++) {
        if (checkLine(i, n, n, currentTurn))
            return true;
    }
    // Check the main diagonal
    if (checkLine(0, n + 1, n, currentTurn))
        return true;
    // Check the other diagonal
    if (checkLine(2, 2, n, currentTurn))
        return true;
    return false;
};
...
const checkWinForNxNBoard = (n, currentTurn) => {
    ...
    // Check for rows
    for (let i = 0; i < n * n; i += n) {
        if (checkLine(i, 1, n - 1, currentTurn))
            return true;
        if (checkLine(i + 1, 1, n - 1, currentTurn))
            return true;
    }
    // Check for cols
    for (let i = 0; i < n; i++) {
        if (checkLine(i, n, n - 1, currentTurn))
            return true;
        if (checkLine(i + n, n, n - 1, currentTurn))
            return true;
    }
    // Check the main diagonal
    if (checkLine(0, n + 1, n - 1, currentTurn))
        return true;
    if (checkLine(1, n + 1, n - 1, currentTurn))
        return true;
    if (checkLine(n, n + 1, n - 1, currentTurn))
        return true;
    if (checkLine(n + 1, n + 1, n - 1, currentTurn))
        return true;
    // Check the other diagonal
    if (checkLine(n - 1, n - 1, n - 1, currentTurn))
        return true;
    if (checkLine(n - 2, n - 1, n - 1, currentTurn))
        return true;
    if (checkLine(n * 2 - 1, n - 1, n - 1, currentTurn))
        return true;
    if (checkLine(n * 2 - 2, n - 1, n - 1, currentTurn))
        return true;
    return false;
};
